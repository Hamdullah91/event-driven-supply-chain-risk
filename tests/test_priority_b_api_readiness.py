...
def test_company_list_supports_frontend_filters():
    ...
    try:
        response = TestClient(app).get(
            "/companies?search=nvidia&industry_id=semiconductors&entity_type=public"
        )
...
def test_agent_response_exposes_typed_partial_provenance():
    app.dependency_overrides[get_agent_service] = lambda: _AgentService()
    try:
        response = TestClient(app).post(
            "/api/v1/agent/query",
            json={"question": "How is NVIDIA exposed?"},
        )
        assert response.status_code == 200
...
def test_detailed_health_reports_truthful_subsystems(monkeypatch):
    app.dependency_overrides[get_neo4j_connection] = lambda: _Connection()
    settings = get_settings()
    monkeypatch.setattr(settings, "LLM_PROVIDER", "")
    monkeypatch.setattr(settings, "NEWS_POLLER_ENABLED", False)
    ...
    try:
        ...
        assert body["services"]["websocket"]["status"] == "available"
        assert body["services"]["event_poller"]["status"] == "disabled"
        assert body["services"]["news_api"]["status"] == "unconfigured"
        assert body["services"]["event_classifier"]["status"] in {
            "available",
            "unavailable",
        }
    finally:
