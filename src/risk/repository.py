...
from src.risk.models import DirectAffectedCompany, SupplyEdge, ThreeHopSupplyPath, TwoHopSupplyPath
...
class RiskRepository:
    ...
    def get_directly_affected_companies(self, event_id: str) -> list[DirectAffectedCompany]:
        query = """MATCH (event:Event {event_id: $event_id})-[:AFFECTS]->(company:Company)
        RETURN event.event_id AS event_id, company.company_id AS company_id,
        company.name AS company_name, event.severity AS severity ORDER BY company.company_id"""
        with self.connection.driver.session() as session:
            records = list(session.run(query, event_id=event_id))
        return [DirectAffectedCompany(event_id=str(r["event_id"]), company_id=str(r["company_id"]), company_name=str(r["company_name"]), severity=str(r["severity"])) for r in records]

    def get_one_hop_downstream(self, company_id: str) -> list[SupplyEdge]:
        query = """MATCH (source:Company {company_id: $company_id})-[r:SUPPLIES]->(target:Company)
    ...
    def get_two_hop_downstream(self, company_id: str) -> list[TwoHopSupplyPath]:
        query = """MATCH (source:Company {company_id: $company_id})-[r1:SUPPLIES]->(hop1:Company)-[r2:SUPPLIES]->(hop2:Company)
    ...
    def get_three_hop_downstream(self, company_id: str) -> list[ThreeHopSupplyPath]:
        query = """MATCH (source:Company {company_id: $company_id})-[r1:SUPPLIES]->(hop1:Company)-[r2:SUPPLIES]->(hop2:Company)-[r3:SUPPLIES]->(hop3:Company)
    ...
    def company_exists(self, company_id: str) -> bool:
        with self.connection.driver.session() as session:
            record = session.run("MATCH (company:Company {company_id: $company_id}) RETURN count(company) > 0 AS exists", company_id=company_id).single()
        return bool(record["exists"]) if record else False

    def event_exists(self, event_id: str) -> bool:
        with self.connection.driver.session() as session:
            record = session.run("MATCH (event:Event {event_id: $event_id}) RETURN count(event) > 0 AS exists", event_id=event_id).single()
        return bool(record["exists"]) if record else False

    def get_company_blast_radius(self, company_id: str, *, max_hops: int = 3) -> list[dict]:
        if max_hops not in {1, 2, 3}: raise ValueError("max_hops must be between 1 and 3.")
        query = f"""MATCH path = (source:Company {{company_id: $company_id}})-[:SUPPLIES*1..{max_hops}]->(target:Company)
    ...
    def get_event_blast_radius(self, event_id: str, *, max_hops: int = 3) -> list[dict]:
        if max_hops not in {1, 2, 3}: raise ValueError("max_hops must be between 1 and 3.")
        query = f"""MATCH (event:Event {{event_id: $event_id}})-[:AFFECTS]->(source:Company)
        ...
        source.company_id AS affected_company_id, source.name AS affected_company_name,
        target.company_id AS target_company_id, target.name AS target_company_name, hop_distance,
    ...
    def get_company_event_exposure(self, company_id: str, *, max_hops: int = 3) -> list[dict]:
        if max_hops not in {1, 2, 3}: raise ValueError("max_hops must be between 1 and 3.")
        query = f"""MATCH (event:Event)-[:AFFECTS]->(source:Company)
    ...
    def get_company_risk_history(self, company_id: str, *, max_hops: int = 3, limit: int = 100) -> list[dict]:
        """Return event exposures in chronological order for historical risk reconstruction."""
        rows = self.get_company_event_exposure(company_id, max_hops=max_hops)
