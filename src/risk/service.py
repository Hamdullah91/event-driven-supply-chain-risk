...
from src.risk.models import OneHopExposure, ThreeHopExposure, TwoHopExposure
...
class RiskPropagationService:
    ...
    def calculate_three_hop_exposure(
        self,
        event_id: str,
    ) -> list[ThreeHopExposure]:
        """Calculate exactly three supply-chain hops from an affected company."""
        ...
        exposures: list[ThreeHopExposure] = []

        for affected in affected_companies:
            initial_risk = severity_to_initial_risk(affected.severity)
            downstream_paths = self.repository.get_three_hop_downstream(
                affected.company_id
            )
            ...
            for path in downstream_paths:
                ...
                exposures.append(
                    ThreeHopExposure(
                        ...
                        hop_2_company_id=path.hop_2_company_id,
                        hop_2_company_name=path.hop_2_company_name,
                        hop_3_company_id=path.hop_3_company_id,
                        hop_3_company_name=path.hop_3_company_name,
                        hop_1_weight=path.hop_1_weight,
                        hop_1_weight_source=path.hop_1_weight_source,
                        hop_2_weight=path.hop_2_weight,
                        hop_2_weight_source=path.hop_2_weight_source,
                        hop_3_weight=path.hop_3_weight,
                        hop_3_weight_source=path.hop_3_weight_source,
                        hop_distance=result.hop_distance,
                        initial_risk=result.initial_risk,
                        path_dependency=result.path_dependency,
                        distance_decay=result.distance_decay,
