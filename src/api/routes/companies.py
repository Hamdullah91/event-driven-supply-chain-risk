...
from src.api.schemas import CompanyDetail, CompanyListResponse, CompanyNetwork, EntitySearchResponse
...
@router.get("", response_model=CompanyListResponse, summary="List and filter companies")
def list_companies(
    service: CompanyService,
    limit: Annotated[int, Query(ge=1, le=500)] = 100,
    offset: Annotated[int, Query(ge=0)] = 0,
    search: Annotated[str | None, Query(min_length=1, max_length=200)] = None,
    industry_id: Annotated[str | None, Query(min_length=1, max_length=200)] = None,
    entity_type: Annotated[str | None, Query(min_length=1, max_length=100)] = None,
) -> CompanyListResponse:
    return CompanyListResponse(**service.list_companies(limit=limit, offset=offset, search=search, industry_id=industry_id, entity_type=entity_type))


@search_router.get("", response_model=EntitySearchResponse, summary="Search graph entities")
def search_entities(
    service: CompanyService,
    q: Annotated[str, Query(min_length=1, max_length=200)],
    limit: Annotated[int, Query(ge=1, le=100)] = 25,
) -> EntitySearchResponse:
    return EntitySearchResponse(**service.search_entities(q, limit=limit))
...
@router.get("/{company_id}/network", response_model=CompanyNetwork, summary="Get a bounded company graph neighbourhood")
