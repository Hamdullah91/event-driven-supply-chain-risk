...
class EventDetail(BaseModel):
    ...
    evidence_status: EvidenceAvailability = EvidenceAvailability.UNAVAILABLE
...
class BlastRadiusCompany(BaseModel):
    ...
    transmission_factor: float = Field(ge=0, le=1)
...
class EventBlastRadiusCompany(BaseModel):
    ...
    initial_risk: float = Field(ge=0, le=1)
    path_dependency: float = Field(ge=0, le=1)
    distance_decay: float = Field(ge=0, le=1)
