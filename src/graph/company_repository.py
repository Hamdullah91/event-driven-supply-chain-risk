...
class CompanyGraphRepository:
    ...
    def get_company_network(
        ...
        *,
        ...
        root_query = """
        MATCH (root:Company {company_id: $company_id})
        RETURN root
        """
        ...
        path_queries = {
            hop: f"""
            MATCH p=(root:Company {{company_id: $company_id}})
                -[:{relationship_pattern}*1..{hop}]-(connected)
            WHERE all(
                node IN nodes(p)
    ...
    def _consume_path(
        ...
        *,
        ...
        for relationship in path.relationships:
            source_id = cls._node_graph_id(relationship.start_node)
            target_id = cls._node_graph_id(relationship.end_node)
            ...
            relationships[relationship_id] = {
                ...
                "properties": cls._json_safe(dict(relationship)),
            }
    ...
    def _consume_node(
        ...
        name = (
            ...
            or properties.get("company_id")
            or properties.get("facility_id")
            or properties.get("product_id")
            or properties.get("material_id")
            or properties.get("technology_id")
            or properties.get("industry_id")
            or properties.get("location_id")
            or properties.get("country_id")
            or graph_id
        )
