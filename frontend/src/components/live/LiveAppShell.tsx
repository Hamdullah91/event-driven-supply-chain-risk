...
import { refetchForRiskUpdate } from "../../realtime/riskStreamInvalidation";
...
export function LiveAppShell({ children, activeSection, onSectionChange, inspectorRef, onInspect, onCloseInspector, onInspectorAction }: Props) {
  ...
  const refreshRiskUpdate = () => {
    if (!notification) return;
    const update = notification;
    void refetchForRiskUpdate(queryClient, update).finally(() => {
      if (useUiStore.getState().newRiskNotification === update) setNotification(null);
    });
  };
  ...
  return <div className="app-shell">
    ...
    <div className="app-main">
      ...
      {notification && <div className="live-risk-notification" role="status"><div><strong>New risk data available</strong><span>Company {notification.companyId}{notification.eventId ? ` · Event ${notification.eventId}` : ""}. Active analysis remains unchanged until refreshed.</span></div><div className="live-risk-notification-actions"><button type="button" onClick={refreshRiskUpdate}>Refresh</button><button type="button" onClick={() => setNotification(null)} aria-label="Dismiss risk update"><X size={14} /></button></div></div>}
