...
import { ArrowRight, Building2, FileText, ShieldCheck, X } from "lucide-react";

import { useCompany, useCompanyRisk, useEvent } from "../../query/hooks";
...
export function LiveEntityInspector({ reference, onClose, onAction }: Props) {
  ...
  const companyId = reference.entityType === "Company" ? reference.id : undefined;
  ...
  const name = company.data?.name ?? event.data?.eventType ?? reference.name ?? reference.id;
  const type = reference.entityType ?? "Unknown";
  const loading = (company.isPending && Boolean(companyId)) || (event.isPending && Boolean(eventId));
  const error = (company.isError && Boolean(companyId)) || (event.isError && Boolean(eventId));
  ...
  return (
    <aside className="entity-inspector" aria-label={`${name} inspector`}>
      ...
      <section className="inspector-identity"><div className="inspector-identity-icon"><Building2 size={20} aria-hidden="true" /></div><div><span>{type}</span><h2>{name}</h2><p>{reference.id}</p></div></section>
      ...
      <div className="entity-inspector-scroll">
        {loading && <section className="inspector-section"><span className="inspector-section-label">LOADING</span><span className="inspector-unavailable">Resolving canonical live entity…</span></section>}
        ...
        {!company.data && !event.data && !loading && !error && <section className="inspector-section"><ShieldCheck size={16} aria-hidden="true" /><span className="inspector-unavailable">No dedicated detail endpoint exists for this entity type. Search identity is preserved without inventing metadata or actions.</span></section>}
      </div>
    </aside>
  );
}
