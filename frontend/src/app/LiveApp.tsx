...
export function LiveApp() {
  const navigate = useNavigate(); const location = useLocation(); const inspectorRef = useUiStore((state) => state.inspectorRef); const setInspectorRef = useUiStore((state) => state.setInspectorRef);
  ...
  const inspectorAction = (action: LiveInspectorAction, ref: InspectorRef) => { if (action === "Open Profile" && ref.entityType === "Company") navigate(`/companies/${encodeURIComponent(ref.id)}`); else if (action === "Open Event" && ref.entityType === "Event") navigate(`/events/${encodeURIComponent(ref.id)}`); else if (action === "Open Company" && ref.context?.relatedCompanyId) navigate(`/companies/${encodeURIComponent(ref.context.relatedCompanyId)}`); else if (action === "Explore Network" && ref.entityType === "Company") navigate(`/network?mode=structure&focusType=Company&focusId=${encodeURIComponent(ref.id)}&depth=1`); else if (action === "Open Impact" && ref.entityType === "Company") navigate(`/network?mode=impact&focusType=Company&focusId=${encodeURIComponent(ref.id)}&maxHops=3`); else if (action === "Open Impact" && ref.entityType === "Event") navigate(`/network?mode=impact&focusType=Event&focusId=${encodeURIComponent(ref.id)}&eventId=${encodeURIComponent(ref.id)}&maxHops=3`); };
  return <LiveAppShell activeSection={sectionFromPath(location.pathname)} onSectionChange={onSectionChange} inspectorRef={inspectorRef} onInspect={setInspectorRef} onCloseInspector={() => setInspectorRef(null)} onInspectorAction={inspectorAction}><Routes>
    ...
    <Route path="/events" element={<LiveEventsPage />} /><Route path="/events/:eventId" element={<LiveEventsPage />} /><Route path="/network" element={<LiveNetworkPage />} />
    <Route path="/companies" element={<LiveCompaniesPage />} /><Route path="/companies/:companyId" element={<LiveCompaniesPage />} />
    <Route path="/risk-analysis" element={<LiveRiskAnalysisPage />} />
